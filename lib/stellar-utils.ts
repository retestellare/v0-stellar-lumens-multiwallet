...
/**
 * Create a new Liquidity Pool by establishing trust and making initial deposit
 * @param secretKey - Secret key of the creating account
 * @param assetACode - Code for asset A
 * @param assetAIssuer - Issuer for asset A (empty string for native XLM)
 * @param assetBCode - Code for asset B
 * @param assetBIssuer - Issuer for asset B (empty string for native XLM)
...
export const createLiquidityPool = async (
  ...
  amountA: string,
  amountB: string
): Promise<{ success: boolean; hash?: string; poolId?: string; error?: string }> => {
  try {
    const server = new Horizon.Server(HORIZON_URL);
    const keypair = Keypair.fromSecret(secretKey);
    const sourcePublicKey = keypair.publicKey();
    ...
    // Create asset objects
    const assetA = assetAIssuer 
      ? new Asset(assetACode, assetAIssuer)
      : Asset.native();
    const assetB = assetBIssuer 
