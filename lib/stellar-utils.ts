...
/**
 * Cancel up to 99 offers in a single Stellar transaction (StellarTerm-style).
 * Each offer is cancelled by adding a manageSellOffer operation with amount=0.
 */
export const cancelAllOffers = async (
  secretKey: string,
  offers: Array<{
    id: string;
    sellingCode: string;
...
): Promise<{ success: boolean; hash?: string; cancelled: number; error?: string }> => {
  if (offers.length === 0) return { success: true, cancelled: 0 };
  const batch = offers.slice(0, 99);
  ...
  try {
    ...
    for (const offer of batch) {
      const sellingAsset =
        offer.sellingCode === 'XLM' || offer.sellingCode === 'native'
          ? Asset.native()
          : new Asset(offer.sellingCode, offer.sellingIssuer);

      const buyingAsset =
        offer.buyingCode === 'XLM' || offer.buyingCode === 'native'
          ? Asset.native()
          : new Asset(offer.buyingCode, offer.buyingIssuer);
    ...
    transaction.sign(keypair);
    const result = await server.submitTransaction(transaction);
    return { success: true, hash: result.hash, cancelled: batch.length };
  } catch (error: any) {
    let errorMessage = error.message || 'Failed to cancel offers';
    if (error.response?.data?.extras?.result_codes) {
      const codes = error.response.data.extras.result_codes;
      errorMessage = codes.operations?.[0] || codes.transaction || errorMessage;
    }
    return { success: false, cancelled: 0, error: errorMessage };
  }
};
